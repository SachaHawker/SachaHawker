...
where a. sub_category  in (413,581,945);
...
create or replace temp table Pies_Party as
    select cust.sr_id
         ,SKU.SKU_desc
...
--          and a.transaction_date >= (current_date-180)
            ...
            and SKU.sub_category  in (413,581,945)

      group by 1,2 ;

select count(*) from Pies_Party; --25234308
select * from Pies_Party limit 5;
...
create or replace temp table Pies_Party_trans as
    select *,
...
from Pies_Party_trans;

--1.76111111000,2.10000000000,2.75000000000

create or replace temp table ASP as
select
    a.sku_key
    ,a.sku
    ...
    and A.sub_category in(413,581,945)
    and asp <> 0
order by asp desc;

select count(*), case when asp between 0.5 and 1.75 then 1
when asp <2.50 then 1
when asp >2.50 then 2
 end as price
