...
select * from CUSTOMER_ANALYTICS.PRODUCT_REPORTING.SH_X01232_FINAL_SEL limit 5;
...
set fin_yr = (select case when $print_start < '2020-03-08' then '1920'
                          ...
                          when $print_start >= '2022-03-06' then '2223'
                            when $print_start >= '2023-03-06' then '2324'
...
select distinct a.sku
                ...
                ,avg(case when (b.margin_12weeks-11.02)>0 then (b.margin_12weeks-11.02) else null end) as margin
...
-- get transaction info
 create or replace temp table produce_trx as
    ...
    inner join EDWS_PROD.PROD_EDW_SAS_ADHOC_VIEWS.vw_c_location_map as lm
        ...
        where transaction_date > dateadd(day,-7*26,'2023-02-16')
        and transaction_date <= '2023-02-16'
...
create or replace temp table CH_Customer_File as
    ...
    where campaigncode = 'C000002611' and flowchartname = '2223_XCAT_PII'
        and contactstatuscode <> 'SUPPRESSED';

select * from CH_SELECTION_FILE limit 10;
-- select count (distinct targeted_skus) from CH_Selection_File;
...
--        trading_profit / red_cost                                   as trading_roi,
       case when red_cost = 0 then 0 else (trading_profit / red_cost) end as trading_roi,
...
select * from CUSTOMER_ANALYTICS.PRODUCT_REPORTING.prod_bau_1 limit 5;
select * from BUDGET_ASSUMPTION limit 5;
